...
export const MORNING_BRIEFING_CONFIG: WorkflowConfig = {
  ...
  description: 'Relatório executivo diário com métricas, alertas e prioridades',
  frequency: 'daily',
  priority: 'high',
  enabled: true,
...
export class MorningBriefingWorkflow {
  ...
  async execute(input: MorningBriefingInput): Promise<WorkflowResult> {
    ...
    try {
      ...
      logger.info('workflow-complete', 'Morning Briefing concluído', {
      ...
      return {
        success: true,
        summary: `Briefing diário gerado com sucesso para ${input.date.toLocaleDateString('pt-BR')}`,
